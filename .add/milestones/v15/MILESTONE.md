...
- [ ] teams-governance-ui         depends-on: design-system-extension, bff-patch-passthrough, teams-add-by-email — `/teams` FRONTEND page: team list/create/delete, set team_budget_usd (PATCH), team detail with member add (BY EMAIL) + remove (all /admin/teams* + members*); owner/admin only; consumes the design system + four state patterns. Presentation-only (the gateway change lives in teams-add-by-email).
