...
rationale: new-major follow-on (v28; project-lead/auto, 2026-06-16, Tin confirmed "full coverage program" at v25 intake). This is a **roadmap stub written to disk for visibility only** — it is intentionally NOT registered as an active engine milestone (a milestone is only ever active or archived; a not-yet-opened one is a planning doc). **Open it with `add.py new-milestone v28 --force`** once the prior milestones land, then re-fill from this sketch through the normal scope-drafting loop (co-specify + human confirm). The provider-config gap is delivered by v25, so it is excluded here. (Renumbered twice on 2026-06-17: v26→v27 when Tin landed the v25 BE config-cleanup follow-ups first; then v27→v28 when Tin chose the billing-precision program as the next concrete milestone — that took v27.)
...
## Status: PLANNED (roadmap stub) — open after the prior milestones land via `add.py new-milestone v28 --force`.
