...
- [ ] The full gateway suite completes green three consecutive times at `-n 12 --dist loadscope` with NO `--reruns`, no manual retry and no chunking workaround        (← suite-stability)   (verify: 3 consecutive full-suite runs in one unbroken sequence, each a single pytest invocation, each exit 0, wall-clock recorded per run; a red run RESETS the streak)
      ...
      <!-- FOUND AT BUILD (flake-tail-burndown): this criterion measures MORE than the frozen task's scope. The sleep-census work is complete and machine-checked (208 sites partitioned, UNKNOWN=0, four AST guards), yet five of six streak attempts died on latent-race classes that are NOT sleep sites: a DB singleton row read without schema ownership, a live DNS lookup in a "no network required" unit test, a poll on the wrong signal, a fire-and-forget assertion with no wait at all, a self-contradicting race assertion, and a settle that cannot distinguish "all landed" from "none started". All nine classes are fixed and each was reproduced deterministically. Treat this criterion as a full latent-race audit of 4566 tests, not as a corollary of the census. -->
      ...
      <!-- Evidence lives in the task's §6. Attempt log to date: A1 red@run1 (6 failures, 2 classes) · A2 green,red@run2 · A3 green,red@run2 · A4 red@run1 · A5 red@run1 · A6 red@run1 (2 failures / 4566, 894s) · A7 in flight (1st of the 2 capped). Per-attempt defect count 6 -> 1 -> 1 -> 1 -> 1 -> 2. -->
