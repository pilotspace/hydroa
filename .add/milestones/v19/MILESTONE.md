...
- [x] streaming-resilience   depends-on: retry-seam-unify, error-aware-fallback  — HIGH-RISK: pre-first-byte streaming retry/fallback (no mid-stream replay)
- [x] cache-controls         depends-on: none                          — per-request cache TTL override + embedding-response cache (per-tenant, billed on miss only)
- [x] semantic-cache         depends-on: cache-controls                — HIGH-RISK: embedding-similarity cache (near-duplicate prompts hit above configurable threshold)
- [x] reliability-verify     depends-on: retry-seam-unify, error-aware-fallback, streaming-resilience, cache-controls, semantic-cache  — live double-pass ×2 + metric assertions + zero-regression floor
...
- [x] A streaming request that fails BEFORE the first byte retries/falls-over transparently; a failure AFTER the first byte keeps today's behavior (no replay) (← streaming-resilience) (verify: pre-first-byte success test + post-first-byte commit test) ✓ DONE 2026-06-15 — 20 streaming tests green (10 helper + 6 router + 4 use-case); HIGH-RISK human gate SIGNED OFF by Tin ("Sign off PASS + commit"); refute-read caught the SYNCHRONOUS circuit-open escape (open_stream moved inside try) — 3 sync-circuit tests added; opt-in default-off byte-identical (GATEWAY_STREAM_RESILIENCE_ENABLED=false → OLD path verbatim, 3 frozen stream tests green); single-bill preserved (only the served attempt extracts usage)
- [x] A repeated identical embeddings request serves from cache (per-tenant isolated, billed only on miss), and a caller can override the cache TTL per request (← cache-controls) (verify: embedding-cache hit/miss + per-request-TTL tests) ✓ DONE 2026-06-15 — 23 tests green (resolve_cache_ttl + build_embedding_cache_key pure fns + 9 use-case cache tests); regression floor green (embeddings/response_caching/semantic/obs/proxy/fallbacks); refute-read EARNED (20 probes); $0-on-hit billing + per-tenant key isolation; chat use_cases.py UNCHANGED (router-side TTL resolution)
- [x] A near-duplicate prompt (semantic similarity above the configured threshold) serves a SEMANTIC cache hit — per-tenant isolated, default-off, threshold-configurable — and a below-threshold prompt misses (← semantic-cache) (verify: above/below-threshold tests + tenant-isolation test) ✓ DONE 2026-06-15 — TRUE embedding-similarity "vector cache" (3rd layer: exact→normalization→vector); the v5 "semantic" layer was normalization-only (binary, already shipped) so this is named vector_*/x_cache="vector_hit" to avoid collision. 27 vector_cache tests green (cosine + RedisVectorCache + embedding-adapter + use-case wiring + default-off byte-identical); HIGH-RISK human gate SIGNED OFF by Tin ("Sign off PASS + commit"); refute-read EARNED-WITH-GAPS → 2 MED gaps FIXED in-loop (request-session→own-session-factory for the after-response store path; embedder adapter now tested incl. bad-shape regression); pure-Python cosine (no numpy), per-(tenant,model) Redis namespace, configurable threshold (default 0.95), embeddings via the gateway's own upstream (internal/unbilled), $0-on-hit billing, fail-safe→miss, opt-in default-off (GATEWAY_VECTOR_CACHE_ENABLED=false → byte-identical)
- [x] Live double-pass green ×2, retry/fallback/cache Prometheus counters increment as expected, and zero behavioral regression on the committed floor (← reliability-verify) (verify: the §6 evidence block + live double-pass log) ✓ DONE 2026-06-15 — live double-pass GREEN ×2 (18/18 each: pass1 run_id=1781508097, pass2 run_id=1781508115) through the Envoy TLS edge; counters observed incremented (gateway_upstream_retries_total{outcome=retried}, gateway_model_fallbacks_total{outcome=context_window} & {outcome=stream_fallover}, gateway_cache_events_total{result=hit} & {result=vector_hit}); no-DB fast floor re-run green + apps/gateway/src diff EMPTY → full non-e2e floor holds; 3 test-only artifacts committed @ 849fced (NO src change)
