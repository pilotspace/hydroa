...
- [ ] A superadmin can Mint and End a session from the dashboard, without calling the API directly   (← impersonation-ui)
- [ ] An ended session's token stops working immediately, not just at its natural TTL expiry   (← impersonation-live-session-guard)
