...
goal: the dashboard runs on Next.js 16 (latest GA, 16.2.9) with ZERO open critical/high npm advisories, all existing behavioral suites green after the breaking upgrade, and a clean production build on the new default bundler (Turbopack)
...
- [ ] next16-upgrade   depends-on: none   — bump next→16.2.9 + react/react-dom→19.2.x + types + eslint-config-next via @next/codemod; migrate next lint→eslint flat config; rename middleware.ts→proxy.ts (behavior-identical guard); async-API audit; postcss>=8.5.10 override; Turbopack build green; 231 tests + tsc + lint clean; npm audit 0 critical/high.

## Exit criteria (observable; map each to the task that delivers it)
- [ ] `apps/dashboard` runs Next.js 16.2.9 (package.json + lockfile resolve next@16.2.x, react/react-dom@19.2.x) (← next16-upgrade) (verify: package.json/lockfile inspection + `next --version`)
- [ ] `npm audit` reports ZERO critical and ZERO high advisories (the 4 Next.js advisories cleared by 16.2.9 + the transitive postcss moderate addressed by the override) (← next16-upgrade) (verify: `npm audit --json` vulnerabilities.critical==0 && .high==0)
- [ ] `next lint` is migrated to `eslint .` with a flat `eslint.config.mjs`, and lint is clean (← next16-upgrade) (verify: `npm run lint` EXIT=0 on the new config)
- [ ] the route guard is `proxy.ts` (middleware.ts removed), behavior byte-identical — unauthenticated → 307 /login, session cookie → passthrough (← next16-upgrade) (verify: proxy.test.ts green; middleware.ts absent)
- [ ] the full v13/v15 behavioral suite stays green at ≥80% coverage and production code is tsc-clean; `next build` succeeds on the default Turbopack bundler (← next16-upgrade) (verify: `vitest run --coverage` EXIT=0 + `tsc --noEmit` clean + `next build` EXIT=0)
