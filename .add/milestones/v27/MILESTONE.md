...
- [x] stream-usage-completeness   depends-on: tiered-token-billing  — guarantee exactly one billed usage record per stream even when the upstream omits/partials the terminal usage frame (documented fallback count + a flagged record); the richer tiered usage survives the SSE extractor.  ✅ DONE 2026-06-18 (gate PASS; full suite 1181 green [only tests/edge live-stack failing]; migration b8e4f1a7c2d5 adds `usage_source TEXT NOT NULL DEFAULT 'frame'`; flagged-$0 fallback `usage_source=stream_fallback`+WARN, pure predicate `stream_usage_is_complete`; refute-read EARNED, 3 NITs closed). 1 follow-up (client-disconnect/GeneratorExit silent-$0) → TASK.md §7.
...
- [x] A chat stream whose upstream omits the terminal usage frame still produces exactly one billed usage record with a recorded fallback marker (never a silent $0).   (verify: pytest apps/gateway/tests/stream_usage_completeness)   (← stream-usage-completeness)
