...
- [x] chat-modality-guard depends-on: preset-capability-validation — extend the coarse `MODEL_MODALITY_MISMATCH` guard to chat/realtime-WS-chat (seeded from a preset-capability-validation SPEC delta: chat had no guard symmetric to images/embeddings/TTS). DONE gate=PASS 2026-07-01 (7 green; full gateway suite ~2143 accounted for, 1 pre-existing unrelated timing flake confirmed via `git stash`). Contract chose a ZERO-new-per-request-I/O design: extended `CatalogProviderResolver`'s existing cached provider map with an optional `modality_loader`/`modality_for()` (additive, backward compatible with the frozen `provider_chat_dispatch` suite), reused via a NEW `ChatModalityLookup` Protocol (kept separate from the frozen `ProviderResolver` Protocol so its ~14 test fakes never change shape). Fail-open-on-unknown-modality was the one contract-freeze judgment call, Tin-approved via AskUserQuestion. Two independent parallel adversarial refute-read subagents ran: correctness/wiring lens (EARNED, with one REFUTED finding — a genuine concurrency race where overlapping `refresh()` calls could tear the paired provider/modality caches across two different cycles — fixed with an `asyncio.Lock` + a regression test proven to fail without the fix and pass with it); security/safety lens (SAFE TO SHIP, 0 hard-stops, one low-severity SPEC delta: the guard doesn't re-validate `FallbackModelRouter`-rewritten candidates, a DX gap not a security hole). A self-inflicted process-management issue (2 refute-read subagents' own pytest runs colliding with the main full-suite run on the shared test DB) caused a misleading first full-suite result (32 failed/13 errors); resolved by re-running single-process (clean).
...
- [x] A preset target that cannot accept the request's input type is rejected with v55's structured 4xx   (← preset-capability-validation; extended to chat/realtime-WS-chat by chat-modality-guard)
...
- skill   : untouched
- book    : untouched
- gateway (backend) : new `tenant_model_presets` table + migration `b5f8a1d4c7e0`; `TenantModelPresetStore` (+ deadlock-retry hardening); preset:alias resolution wired into all 5 entry points (chat, images, embeddings, STT, TTS) + realtime-WS; tenant-scoped admin API (`/admin/presets/...`); coarse `MODEL_MODALITY_MISMATCH` guard on images/embeddings/TTS, extended to chat/realtime-WS-chat via a new cached `modality_for()` lookup on `CatalogProviderResolver` (zero new per-request I/O) + a new `asyncio.Lock` hardening `refresh()` against concurrent-call races
...
- chat-modality-guard : gate=PASS · tests=7 green, full suite ~2143 accounted for (1 pre-existing unrelated timing flake) · residue=1 SPEC delta (fallback-candidate re-validation gap, low severity) · 1 concurrency defect found by refute-read and fixed pre-gate

### Goal met?   (map the evidence back to this milestone's Exit criteria — read before the Exit-criteria boxes are checked)
- [x] each Exit criterion above is satisfied by a Cross-task evidence row or a Ship-by-domain change (cite which) — criterion 1 ← tenant-preset-store + preset-admin-surface rows; criterion 2 ← preset-resolution-ingress row; criterion 3 ← preset-capability-validation + chat-modality-guard rows
- goal: A tenant admin defines named presets that remap model names, selects among multiple via a `name:` prefix, and requests resolve the preset to a concrete model before the router while the fallback seam stays byte-identical — proven end-to-end by the 5 tasks above (store → ingress resolution → admin surface → capability guard on images/embeddings/TTS → the SAME guard extended to chat/realtime-WS-chat), all gate=PASS, full gateway suite green after merging in the one external prerequisite fix this milestone's safety property depended on.

## Release steps   (AI-DEFINED — fill the ordered steps to ship this milestone; engine records, human gate)
> The AI writes the release steps for THIS milestone here (hints, not engine commands). MERGE is one
> small step among them. These feed the release scope (release.md) when the cut is bundled.
- [ ] Tin reviews the 5-commit diff on `feat/v56-model-presets` (line-by-line human review — the one
      outstanding item from every v56 task's gate record)
- [ ] open/update the PR from `feat/v56-model-presets` → `main` (PR #51 already open — push the 5th
      commit to update it), human reviews + merges
