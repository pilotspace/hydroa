...
goal: An enterprise can front its agent fleet (Claude Code / Cowork / Agent SDK / MCP clients) through Hydroa — a native /v1/messages-compatible ingress, MCP connector allow-lists, per-tool-call metering, and session-hour billing — inheriting guardrails, budgets, logs, and invoices.
rationale: roadmap M3 of 3 (Tin-confirmed 2026-07-12) — Track D + the NEW anthropic-messages-ingress task (Anthropic officially supports gateway-fronting since Apr 2026; verified absent in Hydroa). Sequenced after monetization-core because tool/session metering bills through its rails. Detail drafted at activation.
