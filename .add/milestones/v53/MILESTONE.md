...
- MIGRATIONS-BEFORE-BOOT (HARD, inherited from `docker-compose.prod.yml`): schema bootstrap is a Helm-hook Job (pre-install/pre-upgrade) running alembic to head BEFORE the gateway Deployment becomes ready — the gateway never serves against an unmigrated DB.
...
- [ ] a pre-install/upgrade migration Job runs alembic to head before the gateway becomes ready (gateway never serves an unmigrated DB), and every secret is Secret-sourced with fail-fast on unset   (← migration-and-secrets)
