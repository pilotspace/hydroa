...
- [ADD · open] a frozen test from a CLOSED milestone can legitimately go stale when a later task fixes a behavior that test DEFERRED — the principled handling is a tests-phase STRENGTHENING (preserve the true invariant, update only the stale scaffold), surfaced as a Spec delta, never a silent build-time weakening (evidence: v27 test_sd8 `raises(ValueError)` → 200 + null; its docstring pre-authorized the follow-up).
- [TDD · open] pinning a deferred/out-of-scope concern with a test that asserts the CURRENT (buggy) behavior AND names the follow-up in its docstring turns it into an executable breadcrumb that fails loudly the moment the follow-up lands — forcing the update instead of a silent drift (evidence: test_sd8's scope note surfaced the v28 behavior change in the full suite, not in review).
