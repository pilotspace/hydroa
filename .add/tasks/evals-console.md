...
verified:
  ...
  - { by: loop, at: 2026-08-14, act: reopen, to: direction, reason: "focus the build on the verdict-read experience — trim set/case authoring to a follow-up" }
  - { by: "cli", at: 2026-08-14, act: refreeze, authority: process, direction: "sha256:a09b4f0786d59b93" }
  - { by: "cli", at: 2026-08-14, act: brief, authority: process, brief: "sha256:681d18d37063356b" }
  - { by: "process:run", at: 2026-08-14, act: run, authority: process, outcome: PASS, receipt: /tasks/evals-console.d/runs/3.md }
  - { by: "Tin Dang", at: 2026-08-14, act: gate, authority: process, outcome: PASS, receipt: /tasks/evals-console.d/runs/3.md, brief: "sha256:681d18d37063356b" }
advised_by: ui-designer
---
## CARD
goal: read the verdict with per-case drill-down + pin the baseline — UDD loop, verdict-first IA, per-case diff as the signature element (set/case authoring + launch stay on the /v1 API)
...
- M2 The surface is READ + baseline-pin ONLY, and NEVER handles a raw API key (R:RAW_KEY_IN_CONSOLE): GET sets · GET set(+cases) · GET runs · GET run verdict · GET run cases; PUT baseline (pin the reference run — the verdict READ needs it, else every run reads `no_baseline`). Set/case AUTHORING and run LAUNCH BOTH stay on the API-key `/v1` path (surfaced in the UI as "add sets/cases & launch via the /v1 API") — launching also dials upstreams and must bill the launching key as live traffic ([[eval-run-executor]] A1). So the console holds no raw key at rest or in flight and ships a FOCUSED verdict-READ experience; a follow-up may add in-console set/case authoring.
...
## ASSUMPTIONS
- A1 [who] covers: S1 · the request does not say who may read or pin; taking "any authenticated tenant member (the session Identity's tenant) reads the console AND pins its baseline — R7 has no separate eval-admin role, matching the tenant-scoped-any-key /v1 surface; the nav entry sits in the tenant console (not the superadmin Platform allowlist)" -> if wrong, a role gate is missing or the surface is mis-placed. · probe: a member-role session reads the console and pins a baseline; a tenant-B session hitting tenant-A ids gets 404.
...
BACKEND — session-authed control-plane surface (reuses the frozen stores + verdict core):
  ...
  GET  /admin/evals/runs/{run_id}/cases          -> { data:[{eval_case_id,assertion,status,response_text?,reason?,passed?}] }
  PUT  /admin/evals/sets/{id}/baseline {run_id}   -> 200   (pin the reference run — the READ needs it)
  (NO set/case authoring, NO launch here; both stay on /v1. Uniform 404 for absent/cross-tenant; wired in main.py)
...
FRONTEND — the evals console section (Next 16 App Router, TanStack Query + bffGet, tokens-only):
  ...
  components/evals/{EvalsListPage,SetDetailPage,RunVerdictPage,VerdictBanner,CaseDiffList,CaseDiffRow,EvalStatusBadge,PinBaselineControl}.tsx
  nav: components/ui/app-shell.tsx NAV_GROUPS (Insights group, /app/evals, all roles)
  bff types: hand-written interfaces at the bffGet<T> call sites (read-focused; no create/add-case dialogs in v1)
```
scope:
- src/gateway/evals/console/router.py + src/gateway/main.py (include the router) + error_catalog reuse (EVAL_SET_NOT_FOUND / EVAL_RUN_NOT_FOUND)
...
considered-and-rejected: calling /v1/evals from the BFF with the session — /v1 is API-key-only and rejects a session JWT; console-launch — needs a raw key the session must not hold (deferred, user-approved read-first scope 2026-08-13); IN-CONSOLE set/case authoring — trimmed to a follow-up to FOCUS v1 on the verdict-READ experience (user 2026-08-14). Authoring stays on the /v1 API.
...
- E4 an empty sets list → an Empty state that directs the operator to the /v1 API to add sets (no in-console create in the read-focused v1) (A4).
...
- test_admin_evals_session_pins_baseline · covers: M2, A1 · a session Identity PUTs a baseline pin (tenant-scoped); the console exposes NO set/case authoring route.
...
- test_empty_sets_points_to_api · covers: E4 · an empty sets list renders an Empty state directing the operator to the /v1 API (no in-console create).
