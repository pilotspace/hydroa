...
### Competency deltas
- [ADD · folded] a skip-gated live-verify task is NOT red-first — the impl it proves is an already-gated upstream task; the floor is honored by SKIP-not-fail + first-hand real-infra assertion, recorded explicitly in §4 so it is not mistaken for a missing red (evidence: this task ran green immediately against MinIO). [folded foundation-version 38]
- [TDD · folded] pydantic `SecretStr` fields reject a plain `str` under pyright even though they coerce at runtime — wrap test-constructed secrets in `SecretStr(...)` to keep the zero-new-error bar (evidence: live test 48:40 reportArgumentType → fixed). [folded foundation-version 38]
