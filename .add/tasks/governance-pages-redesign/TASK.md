...
- [UDD · folded] the per-page-fit standard (PageHeader everywhere; hero+tabs only where the page warrants) scales the monitoring redesign cleanly to a heterogeneous page set — simple tables stayed header+table, complex pages got tabbed IA, with zero forced/empty tabs (evidence: 6 governance pages shipped under one frozen contract, 794 green). [folded foundation-version 40]
- [TDD · folded] a tab reorg's co-evolution cost is bounded by where the relocated content is TESTED, not how complex the page is — keys (most-wired) needed ZERO co-evolution because its panels are tested standalone and its table is the default tab; routing needed one async-nav helper (evidence: the ⚠ freeze flag was confirmed correct — keys suites untouched). <!-- e.g.  - [DDD · open] the model missed multi-tenancy (evidence: scenario_x failed) --> [folded foundation-version 40]
