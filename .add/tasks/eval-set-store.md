...
```
Target (measurable): the 5 red CHECKS below go green; the ZDR slow-double test asserts on the PERSISTED ROW (0 rows), not the response; cross-tenant + absent set both return byte-identical 404. Boot/wiring confirmed by the router mounting under the app and the new tables migrating clean.
...
## CHECKS
- test_create_set_is_tenant_scoped · covers: M1 · a set created by tenant A is listed for A and absent for B.
- test_create_case_persists_payload_and_assertion · covers: M2 · a case stores request_body + assertion under the right set/tenant; re-fetch after commit proves the write survived the session boundary.
...
red-first: every check MUST fail first (the `evals/` module does not exist yet — all five are red at authoring).
