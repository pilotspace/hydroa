...
### Competency deltas
- [UDD · open] The a11y guarantee (reduced-motion) belongs in a GLOBAL css net (covers everything unconditionally), while the per-component primitive (Reveal) is the opt-in polish — separating "guarantee" from "enhancement" keeps the invariant robust even if a component forgets the motion-safe gate (evidence: M1 net independent of M2 Reveal).
- [TDD · open] `import.meta.url` is NOT a file:// URL under the jsdom/vitest transform — read repo files in tests via `resolve(process.cwd(), …)` instead (evidence: test_globals_has_reduced_motion_net threw "URL must be of scheme file" → fixed).
