...
Must:
<must>
  - M1 A new `MeteringToolCallObserver` (implements the FROZEN `ToolCallObserver` Protocol verbatim: `record(*, call_id, tenant_id, key_id, server_host, tool_name, status, latency_ms)` — `call_id: UUID` per CR-1/contract v2; this literal originally cited the pre-CR-1 shape, redlined 2026-07-14 to match the shipped Protocol) is the ONLY production wiring for `ToolCallObserver` once both this task and `mcp-connector-passthrough` are built — replacing the sibling's `NoopToolCallObserver` default in `main.py`'s DI graph, constructed with the SAME `RecordingUsageRecorder` instance already wired for every other billed call (no second Redis/session instance).
...
- [x] DECIDED (Tin 2026-07-14): CR-1 opened and applied — `call_id: UUID` added to the upstream Protocol (v2); this task dedupes on a call_id-derived deterministic event id. Exactly-once is structural, no residual.
  - ⚠ POST-FREEZE CORRECTION (verify, 2026-07-14, conf 0.9): the SHIPPED dedupe is NOT the contract's "recorder ON CONFLICT DO NOTHING on a deterministic id" (recorder mints uuid4 internally with no call_id passthrough, and scope forbade widening its frozen signature) — it is a **Redis SETNX in `MeteringToolCallObserver` that FAILS OPEN**: under a Redis outage a genuine duplicate `record()` invocation double-bills. So exactly-once is NOT unconditionally "structural, no residual" as worded above — the residual is "double-bill during a Redis outage + a real duplicate fire" (a narrow window; a missed bill was judged worse than a rare double-bill, so fail-open was chosen deliberately and is tested: `test_dedupe_check_failure_fails_open_and_still_bills`). Carried to the milestone risk register; a DB-level backstop (unique index on a call_id-derived tag) is the follow-on fix, not a 0.9.0 blocker (verify's own recommendation). See [[agent-gateway-r1-milestones]].
