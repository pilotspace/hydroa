...
- [UDD · open] Deterministic read errors (403/404) must set retry:false on the query — the OIDC tab had it (404=unconfigured) but Cache/Guardrails didn't, so a settled 403 would retry-storm before the inline alert (evidence: retry:false added to all three settings queries for parity).
- [ADD · open] The re-cross ritual (phase tests → advance → advance) is the correct mechanism to STRENGTHEN already-green tests mid-verify without tripping build_tampered — used here to add 4 security assertions whose behavior already held (evidence: tripwire held, 16/16 still green).
