...
Coverage target: 100% of `retention_policy.py` + `retention_policy_router.py` (new modules); every new branch in `retention_sweep.py`'s 3 additive passes exercised at least once; frozen `test_retention_sweep.py` (8/8) held byte-identical as the regression floor.

Plan (one test per scenario, asserting behavior not internals):
<test_plan>
  - test_get_default_policy: GET with no override → window_days=null, zdr_enabled=false, effective_window_days inherits operator defaults for all 7 tables, audit_events absent from the map · covers: M1
  - test_put_shortens_window_and_sweep_purges_seven_tables: PUT window_days=30 → response echoes 30 for all 7 tables; arrange aged(50d)/recent(5d) rows in all 7 tables (usage_records, alert_events, artifacts, conversations, memories, batch_job_items, video_generation_jobs) under generous 90/365d operator defaults; sweep_once(); assert only the aged rows are purged · covers: M2, M3, M4
  - test_put_window_above_ceiling_rejected: PUT window_days=400 → 422 ERR_RETENTION_WINDOW_INVALID + GET confirms no persistence · covers: R1
  - test_put_window_nonpositive_rejected: PUT window_days=0 → 422 ERR_RETENTION_WINDOW_INVALID + GET confirms no persistence · covers: R1
  - test_put_non_owner_forbidden[admin|operator|billing_admin|viewer|member]: PUT as each non-owner role → byte-identical 403 ERR_AUTH_FORBIDDEN + no persistence · covers: R2
  - test_zdr_blocks_artifact_upload: zdr_enabled=true tenant POSTs an artifact → 403 ERR_ZDR_PAYLOAD_BLOCKED, zero ArtifactRow created · covers: M5, R3
  - test_zdr_blocks_conversation_memory_batch_video: same tenant attempts conversation/memory/batch/video creation → each independently 403 ERR_ZDR_PAYLOAD_BLOCKED, zero rows in any of the 4 tables · covers: M5, R3
  - test_zdr_completion_skips_cache_but_bills_usage: zdr_enabled=true + cache_enabled=true tenant POSTs 2 identical completions → byte-identical bodies, upstream.calls==2 (cache fully skipped), zero resp-cache:/vec-cache: keys, 2 usage_records rows with accurate tokens/cost · covers: M6, M8
  - test_zdr_purge_pre_existing_payload_rows: tenant with existing inline+s3 artifacts, conversation+message, memory PUTs zdr_enabled=true → sweep purges all rows, ObjectStore.delete() called for the s3 key, conversation_messages gone via CASCADE, resp-cache:/vec-cache: keys purged, a data.purge audit event recorded per purged table · covers: M7
  - test_zdr_purge_survives_restart_mid_purge: a tenant with a partially-purged table (simulating a crash mid-purge) → a second independent sweeper instance ("process restart") finishes the purge idempotently, zero errors · covers: M7 (design-for-failure)
  - test_zdr_purge_object_store_outage_deferred: an s3-backed artifact whose ObjectStore.delete() raises ObjectStoreUnavailableError → that row is deferred (not dropped), sweep_once() never raises, the next tick (once the store recovers) purges it · covers: M7 (design-for-failure)
  - test_cross_tenant_retention_policy_404: a JWT whose tenant_id resolves to no tenants row (orphaned identity) → GET and PUT both 404 ERR_TENANT_NOT_FOUND, no other-tenant data disclosed · covers: R5
  - test_audit_events_never_affected_by_tenant_policy: a tenant with window_days=1 and zdr_enabled=true has a 100d-old audit_events row → survives a sweep under a 730d/365d-floor operator audit window; GET's effective_window_days never includes "audit_events" · covers: R4
  - test_disable_zdr_does_not_restore_purged_rows: a purged memory row stays gone after zdr_enabled is toggled back to false; new writes are accepted again once disabled · covers: M7 (edge case)
  - test_is_zdr_read_port: is_zdr() returns false pre-enable, true post-enable, false for an unknown tenant_id (fail-open, never raises) · covers: M9
</test_plan>
...
Strategy actually used: Steps 1-7 as planned, with 3 build-time refinements surfaced by the tests (recorded here, not silently absorbed):
  (a) Step 6's "per-tenant SHORTENING pass" is expressed as `min(tenant_window, operator_default) * interval` in SQL (not two Python-computed cutoffs) so it stays a provable no-op whenever a tenant's window ≥ the operator default — the original 3 unconditional DELETEs remain the outer bound for every tenant, satisfying "shorten only, never lengthen" by construction rather than by a runtime branch.
  (b) The ZDR purge pass (step 6) was rewritten mid-build from one bulk "all zdr_enabled tenants at once" DELETE per table to a per-tenant loop (SELECT zdr tenant ids, then delete scoped to each `tenant_id`) — the frozen scenario at TASK.md:173 ("each table's purge (>0 rows deleted) emits a record_audit(action='data.purge') event scoped to that tenant") needs a per-tenant rowcount to attribute the audit event to, which the bulk form could not produce. This is additive within the same pass, not a scope change.
  (c) The tenant-scoped `data.purge` audit event keeps `AuditEvent.tenant_id=None` (not the tenant's id) and instead carries `metadata={"tenant_id": ..., ...}` — discovered mid-build that the pre-existing, unrelated `audit_missing_actor` invariant (`gateway/audit/domain/audit_event.py`, frozen by a different, earlier milestone) rejects any `tenant_id`-bearing `AuditEvent` with no `actor_user_id`/`actor_key_id`, which the background sweeper structurally has neither of. Reconciled against TASK.md's own M10 wording ("system-level for the sweeper-driven bulk purge") — tenant_id=None + metadata satisfies "scoped to that tenant" (queryable) without violating the actor invariant. Not a change to this task's own frozen §3 contract.
Two additional test-only fixture decisions (no production code impact): a `tests/retention_zdr/conftest.py` overriding the `app` fixture to run a fast `UsageLedgerFlusher` background task (mirrors `tests/response_caching/conftest.py`'s pre-existing precedent — needed because `httpx.ASGITransport` never triggers the app lifespan, so fire-and-forget usage-recording never reaches Postgres without it), and an `alert_tenant_column` fixture that `ALTER TABLE alert_events ADD COLUMN IF NOT EXISTS tenant_id UUID NULL` — turned out to be a no-op safety net once inspection showed `AlertEventRow` already maps `tenant_id` (nullable, FK omitted only) so `Base.metadata.create_all` already provisions the column; kept for documentation/defensiveness, harmless either way.
