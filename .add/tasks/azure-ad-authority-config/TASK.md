...
- [DDD · folded] a partially-wired seam can hide for a whole milestone: AzureADConfig.authority + _token_url() consumed the authority since v21, but resolve never sourced it — the field looked configurable but wasn't. Evidence: the carries-authority test was the only thing that exposed the gap. Lesson: an end-to-end test (settings→URL) catches "looks wired, isn't" that a unit test on either end misses.
- [TDD · folded] keeping the invariant guards (default-fallback, gating-unchanged) GREEN from the start while only the new-behavior tests go RED cleanly separates "new capability" from "must-not-regress" in the same suite. Evidence: 2 red / 3 green at the red run, all 5 green after a 2-line change.
