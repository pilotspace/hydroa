...
- [ADD · open] A frozen-contract clause can be satisfied by a more-robust SUPERSET of its literal wording (C1: detect-tool-calls vs detect-finish_reason). Honest path = record the deviation as a SPEC delta, not silently edit the contract (evidence: refute-read flagged the literal mismatch; behaviour is correct).
