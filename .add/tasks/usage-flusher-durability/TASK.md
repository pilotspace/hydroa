...
- [SDD · open] A "bound EVERY X call" Must must be verified against EACH call in the path, not the one the pseudocode illustrates — first build wrapped only XADD and left the three advisory `incrbyfloat` awaits bare, partially missing the frozen §1 B4-timeout Must; caught by the pre-gate advisor review, not by tests (no test asserted the advisory-call timeout). Evidence: recorder.py advisory block.
- [TDD · open] "best-effort" and "bounded" are orthogonal properties — a call can be both. The §3 pseudocode's "advisory stays best-effort" was mis-read as "advisory is timeout-exempt". Add a scenario/test that asserts a hung advisory call is bounded (not just swallowed) so this can't regress silently. Evidence: the gap was invisible to the green suite.
