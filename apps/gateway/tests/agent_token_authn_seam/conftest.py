"""Suite-local fixtures for agent-token-authn-seam tests (TASK.md §4).
...
Redis isolation: clears agent_oauth:poll:* and agent_oauth:authz:rl:* keys +
usage:spend:key:* spend counters before each test.
"""
...
async def mint_agent_token(
    ...
    *,
    ...
    # Mint (uses a fresh session to avoid state issues)
    async with app.state.sessionmaker() as session:
...
@pytest.fixture
async def active_embed_model(db_session: AsyncSession) -> str:
    """Insert a minimal active embedding model."""
    from sqlalchemy import text
    ...
    await db_session.execute(
        text(
            ...
            " VALUES (:i, :n, 8191, true, 'embedding', 'openrouter')"
            " ON CONFLICT (id) DO NOTHING"
        ),
        {"i": model_id, "n": "text-embedding-3-small"},
    )
    await db_session.commit()
    return model_id
