...
- [x] devtool-vitest4-upgrade            depends-on: bff-test-harness-strict-handlers  — vitest 3→4 + @vitejs/plugin-react 4→6 + vitest-axe replacement; full npm audit 0 critical/high; 236-test floor green on vitest 4. risk:high → conservative/human-gated. — DONE, gate PASS 2026-06-14 (Tin "PASS — commit it"; vitest 4.1.8 + coverage-v8 4.1.8 + plugin-react 6.0.2 → vite 8; vitest-axe→local test-support/axe.ts shim on axe-core; npm audit 5h+2c → 0/0/0/0; 255 tests/35 files green; eslint 0/0; tsc 0; NO test assertion changed; axe shim probe-verified non-vacuous; coverage metric 94→88% lines is a vitest-4 v8-remapping shift, still >80 threshold).
- [x] realbrowser-a11y-pass              depends-on: none                              — minimal Playwright + viewport axe pass proving color-contrast + true-layout a11y jsdom can't; discharges the standing v13/v15 residue. — DONE, gate PASS 2026-06-15 (Tin scope "primary surfaces + fix findings"; standalone Playwright harness, headless Chromium 1280×800, 5 surfaces /login+/usage+/keys+/spend+/settings via cookie-seed + page.route() stubs; 5/5 green incl. real color-contrast; 2 REAL findings fixed at source — document-title on /usage+/spend, aria-hidden-focus on the recharts chart; floor 255 + audit 0/0 + eslint 0/0 + tsc 0 untouched; scope-gate drift self-healed honestly).
...
- [x] the FULL `npm audit` reports 0 critical and 0 high (dev-toolchain advisories cleared); the 236-test floor stays green on vitest 4 (← devtool-vitest4-upgrade) (verify: `npm audit --json` critical==0 && high==0 + `vitest run --coverage` EXIT 0) — MET: `npm audit` = 0 critical / 0 high / 0 moderate / 0 low (was 5 high + 2 critical, all rooted in esbuild GHSA-gv7w-rqvm-qjhr, cleared at the root by the vite-8 bump); `vitest run --coverage` EXIT 0 with 255 tests / 35 files green on vitest 4.1.8; eslint 0/0; tsc 0; no test assertion weakened (vitest-axe → local axe-core shim, import-only swap). Human-gated (risk:high) PASS by Tin 2026-06-14.
- [x] a real-browser (Playwright + viewport) axe pass runs green over the primary surfaces, proving color-contrast + true-layout a11y (← realbrowser-a11y-pass) (verify: the headless-Chromium axe pass EXIT 0) — MET: `npm run test:a11y` → headless Chromium (Chrome Headless Shell 148) at 1280×800, 5/5 surfaces (/login + /usage + /keys + /spend + /settings) axe-clean (serious/critical == 0) with color-contrast ENABLED (real CSS — the v13 tokens are contrast-clean), EXIT 0. Authed pages rendered via cookie-seed + page.route() stubs (each asserts its real <h1> so the pass is non-vacuous). Two REAL findings the jsdom baseline never saw were fixed at source (document-title; recharts aria-hidden-focus). Standalone harness — the committed vitest floor (255) stays green without Chromium. CI is billing-blocked, so this is fresh LOCAL evidence (v14 precedent).
