...
- [x] gemini-embed-tokens        depends-on: none                                            — replace the chars/4 estimate in `_gemini_embed_to_openai` with an EXACT Gemini-embed token count (mechanism settled at freeze); bill on real tokens; estimate becomes a documented last-resort fallback; non-Gemini billing untouched.
- [x] empty-key-boot-guard       depends-on: none                                            — add a startup guard rejecting a configured-yet-EMPTY upstream API key with a clear boot error (closes the opaque "Bearer ''" 500 seen live v7+v8); absent key = provider disabled (allowed), empty key = boot failure.
- [x] nonchat-soft-budget-alert  depends-on: none                                            — extend `NonChatGovernance` to fire the soft-budget alert via the shared `persist_soft_budget_alert`/`alert_events` seam (explicit reflection-free DI); embeddings/images/audio surface a crossing identically to chat; HARD 402 unchanged.
- [x] test-db-isolation          depends-on: none                                            — make the full `-m 'not e2e'` suite deterministic via a surgical group-preserving Redis clear (XTRIM usage:events + DEL usage:spend:*) in a global autouse fixture, killing the FK-violation flake; document `make test-fast` for per-change gating; no production source change.
- [x] v12-live-verify            depends-on: gemini-embed-tokens, empty-key-boot-guard, nonchat-soft-budget-alert — live double-pass through the TLS edge: exact Gemini-embed tokens billed (not estimated), non-chat soft-budget alert event written, empty-key boot guard rejects at startup; two consecutive clean passes (11/11 ×2).

## Exit criteria (observable; map each to the task that delivers it)
- [x] A Gemini embeddings request bills on an EXACT token count (usage row matches a real count, not `ceil(chars/4)`); non-Gemini billing is unchanged (← gemini-embed-tokens) — LIVE C1: prompt_tokens=42 exact
- [x] Booting the gateway with a configured-yet-empty upstream API key fails fast with a clear boot error; an absent key leaves that provider cleanly disabled (← empty-key-boot-guard) — LIVE C3: empty→exit1, absent→boots
- [x] An embeddings/images/audio request that crosses a key's soft budget writes a `soft_budget_exceeded` alert_event (same dedupe semantics as chat); HARD 402 still enforced (← nonchat-soft-budget-alert) — LIVE C2: 1 row, idempotent, 200
- [x] The full `-m 'not e2e'` suite runs deterministically (no FK-violation flake) across repeated runs; `make test-fast` is documented (← test-db-isolation) — 2 clean full runs (730 passed ×2)
- [x] All behavioral items above proven LIVE through the TLS edge, two consecutive clean passes (← v12-live-verify) — double-pass 11/11 ×2 (run_ids 1781346461, 1781346491)
